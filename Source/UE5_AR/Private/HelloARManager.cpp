...
void AHelloARManager::BeginPlay()
{
	...
	GetWorld()->GetTimerManager().SetTimer(Ticker, this, &AHelloARManager::StopTracking, 10.0f, false, 10.0f); 
}
...
void AHelloARManager::ConfigurePlanes(bool IsVisible)
{
	...
	{
		Planes[i]->SetActorHiddenInGame(!IsVisible);
	}
}

void AHelloARManager::StopTracking()
{
	bStoppedTracking = true;
	GEngine->AddOnScreenDebugMessage(-1, 2.0f, FColor::White, TEXT("Stopped Tracking New Planes"));
}

void AHelloARManager::UpdateTrackedPlanes()
{
	for (const TPair<UARPlaneGeometry*, AARPlaneActor*>& pair : PlaneActors)
	{
		pair.Value->UpdatePlanePolygonMesh();
	}
}
